...
//     const std::string header("[03/08/2018 12:00:00 node1234.example.com generic.workqueue
//     123456] {}"); fmt::print("Header: {}\n", header); const char *begin = &header[0]; const
//     char *end = begin + header.size(); scribe::MessageHeaderParser parser;
//     scribe::MessageHeader msg = parser(begin, end);
//     utils::TimePrinter printer("%Y-%m-%d %H:%M:%S");
//     printer(msg.timestamp);
//     fmt::print("{0} <---> [{1} {2} {3} {4}]\n", header, printer.buffer, msg.server, msg.pool,
//     msg.pid); CHECK(strcmp(printer.buffer, "2018-03-08 12:00:00") == 0);
...
//     {}"); const std::string header2("[03/08/2018 13:00:00 node123456.example.com
//     generic.work.cron 123456] {}"); const char *begin = &header1[0]; const char *end = begin
//     + header1.size(); scribe::MessageHeaderParser parser; scribe::MessageHeader msg1 =
//     parser(begin, end); scribe::MessageHeader msg2 = parser(&header2[0], &header2[0] +
//     header2.size());
